...
"一本书的旅行"校园阅读活动系统交付仓库，包含学生端、管理端、后端服务，以及图书导入样例和站点素材。
...
4. 已有旧系统升级到新版时，先解压新版到新目录，再双击新版目录里的 `upgrade.bat`
...
如果旧系统已经在运行并且里面有真实图书、留言和上传素材，不要直接用新压缩包覆盖旧目录。

推荐步骤：

1. 关闭旧系统的后端、学生端和管理端窗口
2. 把新版压缩包解压到一个新目录
3. 双击新版目录里的 `upgrade.bat`
4. 按提示输入旧系统目录，例如 `D:\library-management-system`
5. 脚本会自动复制旧系统的 `dev.db*`、`uploads` 和 `.env`，然后安装依赖、同步数据库结构并重新构建前端
6. 看到脚本输出的 `reviews=` 数量确认评语仍在，再双击新版目录里的 `start.bat`

升级脚本不会修改旧系统目录；如果升级失败，旧系统目录仍可作为回退来源。
