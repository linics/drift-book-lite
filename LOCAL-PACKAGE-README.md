...
- 学生名单文件（后端导入学生身份时使用，缺失时后端仍可启动但学生身份校验将失败）：
  - **Docker 部署**：名单必须放在项目根目录，文件名保持 `2025学年学生信息.xls`，`docker-compose.yml` 直接挂载该路径
  - **无 Docker 部署**：优先使用 `package-data\student-roster.xls`；不存在时自动回落到根目录同名文件
